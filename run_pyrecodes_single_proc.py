...
import sys
#sys.path.append("/work2/07059/jyzhao/stampede3/run_pyrecodes/pyrecodes")
sys.path.append(str(Path(__file__).resolve().parent / "pyrecodes"))  # yhy
...
def run_one_realization(main_file, rlz, rwhale_run_dir, system_config, save_pickle_file):
    ...
    if DO_MAP_PLOTS:
        ...
        for time_step in time_step_list:
            ...
            if legend is not None:
                legend.set(loc='center left', bbox_to_anchor=(1, 0.5))
            plt.savefig(
                f'status_at_time_step_{time_step}.png',
                dpi=300, bbox_inches='tight', transparent=False, pad_inches=0
            )
            plt.close(fig)
        # create png plot
        geo_visualizer.create_recovery_gif(
            time_step_list,
            file_name='status_at_time_step_TIME_STEP.png',
            fps=2
        )
    ...
    if 'FunctionalHousing' in system_config['Resources']:
        resources_to_plot.append('FunctionalHousing')
        units_to_plot.append(system_config['Resources']['FunctionalHousing'].get('Unit', 'unit_FunctionalHousing'))
...
def run_pyrecodes(  # noqa: C901
    ...
    os.environ["pyrecodes_input_dir"] = str(input_data_dir)
    ...
    if damage_input['Type'] == 'SpecificRealization' or \
        ...
        for rlz in rlz_to_run:
            ...
            rlz_run_dir = output_dir / f'workdir.{rlz}'
            rlz_run_dir.mkdir()
